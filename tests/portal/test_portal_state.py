...
from services.workspace.workspace import deep_analyze_issue, init_workspace, sync_workspace_run_prefect_state, workspace_paths
...
class PortalStateTest(unittest.TestCase):
    ...
    def test_portal_state_can_use_workspace_run_artifacts_for_workbench(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            report_tab = next(tab for tab in workbench["detail_tabs"] if tab["id"] == "report")
            self.assertIn("section_outputs/rca.json", report_tab["content"])
    ...
    def test_portal_state_surfaces_prefect_runtime_information(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            run_payload = deep_analyze_issue(temp_dir, "SSD-102", policies=["team:ssd"])
            run_name = Path(run_payload["run_dir"]).name
            sync_workspace_run_prefect_state(
                temp_dir,
                run_name,
                prefect_state="Completed",
                requested_by="alice",
                flow_run_id="flow-run-789",
