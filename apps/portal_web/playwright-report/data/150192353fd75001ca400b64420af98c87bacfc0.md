...
- Name: sidebar-navigation.spec.ts >> Sidebar Navigation Tests >> should navigate to Analyze page
- Location: e2e\sidebar-navigation.spec.ts:70:3
...
Call log:
  - waiting for locator('aside.nav a[href="/"]')
...
```ts
  ...
  2   | 
  3   | /**
  4   |  * E2E Test: Sidebar Navigation
  5   |  *
  6   |  * 测试所有侧边栏入口的导航功能
  7   |  * 验证每个页面是否正确加载
  8   |  */
  9   | 
  10  | test.describe('Sidebar Navigation Tests', () => {
  11  |   const runnerToken = 'test-token-123';
  12  | 
  ...
  19  | 
  20  |     // Setup runner connection if needed
  21  |     const tokenInput = page.locator('input[placeholder="change-me"]');
