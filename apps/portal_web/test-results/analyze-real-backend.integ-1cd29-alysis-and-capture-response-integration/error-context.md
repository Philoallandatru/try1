...
- Name: analyze-real-backend.integration.spec.ts >> Analyze Page - Real Backend Integration >> should attempt real analysis and capture response
- Location: e2e\analyze-real-backend.integration.spec.ts:56:3
...
Call log:
  - waiting for locator('input[placeholder*="SSD-DEMO-A"]') to be visible
...
> 60  |     await page.waitForSelector('input[placeholder*="SSD-DEMO-A"]', { timeout: 10000 });
  ...
  136 | 
  137 |     if (resultCount > 0) {
  138 |       console.log('SUCCESS: Result surface displayed');
  139 | 
  140 |       // Capture result details
  141 |       const summaryHeading = page.locator('.result-surface h3');
  142 |       if (await summaryHeading.count() > 0) {
  143 |         const title = await summaryHeading.textContent();
  144 |         console.log('Analysis title:', title);
  145 |       }
  146 |     } else {
  147 |       console.log('No result surface found');
  148 |     }
  149 | 
  150 |     // Check for empty state
  151 |     const emptyState = page.locator('.empty-state');
  152 |     const emptyCount = await emptyState.count();
  153 | 
  154 |     if (emptyCount > 0) {
  155 |       const emptyText = await emptyState.textContent();
  156 |       console.log('Empty state:', emptyText);
  157 |     }
  158 |   });
  159 | 
  160 |   test('should check backend API endpoints directly', async ({ page, request }) => {
```